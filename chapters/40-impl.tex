...
\begin{figure*}
  \begin{tikzpicture}[
    ...
    \begin{scope}[node distance=1cm]
      ...
      \node[right=1.4cm of read,mod] (ec) {\texttt{mnesia\_ec} (\cref{subsec:bg mnesia arch,subsec:impl api})};
...
Apart from the \acrlong{awset}, a \acrlong{rwset} (\cref{sec:impl overview}) 
...
\begin{figure}[htp]
  ...
  \begin{subfigure}[t]{0.3\textwidth}
    ...
    \begin{tikzpicture}[
      ...
      \node[left=1mm of s3-m1] (s3-w1) {\color{Red}{Error!}};
...
\begin{figure}[htp]
  ...
  \begin{subfigure}[t]{0.3\textwidth}
    ...
    \begin{tikzpicture}[
      ...
      \node[left=1mm of s2-m2] (s2-o1) {add(y)\texttt{->}A};
...
By the property of op-based \acrshortpl{crdt} (\cref{prop:op crdt convergence}),
...
but the remove-wins semantics is similar (\cref{appendix:rwset}). To achieve
...
in terms of its \acrshortpl{api} to minimise the amount of code refactoring 
needed (\cref{subsec:impl api}). We also examined the suitability 
of different \acrshortpl{crdt} and decided to use pure op-based \acrshortpl{crdt}
