...
of Mnesia (\cref{sec:bg mnesia}) before giving an overview of \acrshortpl{crdt}.
...
\begin{listing}[htp]
\centering
\begin{minted}[xleftmargin=30pt]{erlang}
  -record(student, 
    {id :: integer(), 
    name:: string(), 
    college :: string(),
...
A state-based \acrshort{crdt} communicates by propagating its state of 
...
A family of data structures, (\acrshortpl{crdt}) are also described (\cref{sec:bg crdt})
