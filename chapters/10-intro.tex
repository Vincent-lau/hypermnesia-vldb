...
The rest of the report is organised as follows: \Cref{sec:bg} introduces the
reader to the background knowledge, including the architecture of Mnesia and
different ways of achieving eventual consistency. \Cref{sec:related} describes
recent research on eventually consistent databases and \acrshortpl{crdt}: a data
structure for implementing eventually consistent systems. \Cref{sec:impl} goes 
into detail about designing an API for Hypermnesia and implementing it with practical optimisations.
\Cref{sec:eval} evaluates Hypermnesia in terms of its correctness,
performance and usability. \Cref{sec:concl} concludes the
